...
folds = createFolds(trainData$Churn, k = 10)
cv = lapply(folds, function(x) {
  train_fold = trainData[-x, ]
  test_fold = testData[x, ]
  classifier = glm(Churn ~ .,family=binomial(link="logit"),data=trainData)
  y_pred = predict(classifier, newdata = test_fold[])
  y_pred = (y_pred >= 0.5)
  cm = table(test_fold[, 19], y_pred)
